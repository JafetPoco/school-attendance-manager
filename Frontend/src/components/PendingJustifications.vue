  ...
  <div class="space-y-6">
    ...
    <div v-if="!loading && !errorMessage && pendingJustifications.length === 0" 
      ...
      <p class="text-sm text-slate-500">
        {{ hasActiveFilters ? 'No se encontraron justificaciones con los filtros aplicados' : 'Todas las justificaciones han sido procesadas' }}
      </p>
...
const emit = defineEmits<{
  ...
  (e: 'show-success', message: string): void
...
const hasActiveFilters = computed(() => {
  return filter.value.dateFilter !== 'TODAY' || searchQuery.value !== ''
})
...
const refreshList = async () => {
  ...
  toast.showInfo('Actualizado', 'La lista de justificaciones se ha actualizado', 2000)
}
...
const approveJustification = async (item: JustificationResponse) => {
  ...
  try {
    ...
    if (response.success) {
      ...
      emit('show-success', `Justificación de ${item.studentName} aprobada correctamente`)
      await loadJustifications()
    } else {
      toast.showError('Error', response.error.message)
      emit('show-error', response.error.message)
    }
  } catch (error) {
    const msg = error instanceof Error ? error.message : 'Error al aprobar la justificación'
...
const rejectJustification = async (item: JustificationResponse) => {
  ...
  try {
    ...
    if (response.success) {
      ...
      emit('show-success', `Justificación de ${item.studentName} denegada`)
      await loadJustifications()
    } else {
      toast.showError('Error', response.error.message)
      emit('show-error', response.error.message)
    ...
    const msg = error instanceof Error ? error.message : 'Error al denegar la justificación'
    toast.showError('Error', msg)
    emit('show-error', msg)
  } finally {
    processingId.value = null
...
const loadJustifications = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      emit('update-count', totalJustifications.value)
    } else {
    ...
    emit('show-error', errorMessage.value)
  } finally {
    loading.value = false
  }
}
